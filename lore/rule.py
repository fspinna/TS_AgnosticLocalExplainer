...
from lore.util import vector2dict, multilabel2str
...
def check_feasibility_of_falsified_conditions(delta, unadmittible_features):
    for p in delta:
        if p.att in unadmittible_features:
            if unadmittible_features[p.att] is None:
                return False
            else:
                if unadmittible_features[p.att] == p.op:
                    return False
    return True
